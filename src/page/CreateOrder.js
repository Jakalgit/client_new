...
const CreateOrder = () => {
    ...
    item.basketItems.map(item => {
        for (let i = 0; i < item[0]; i++) {
            window.goods.push({length: Number(item[2].length), width: Number(item[2].width), height: Number(item[2].height), weight: Number(item[2].weight)})
        }
    })
    ...
    return (
        ...
        <div>
            {item.basketItems.length !== 0 && window.goods.length !== 0 ?
                <div>
                    <div className={CreateOrderCss.create_section}>

...
const Filters = (props) => {
    ...
    const drawCategory = () => {
        ...
        if (item.downCategories && currentCategoryId !== -1) {
            let mas = []
            item.downCategories.forEach(el => el.categoryId === currentCategoryId ? mas.push(el) : null)
    ...
    const changeCurrentAvailability = (e) => {
        item.setCurrentAvailability(e.target.checked)
    }
                ...
                <div className="container">
                    <div className="row">
                        <div className={FiltersCss.out_of_stock}>
                            <input type="checkbox" className={FiltersCss.checkbox} onChange={(e) => changeCurrentAvailability(e)} checked={item.currentAvailability}/>
                            <h2 className={FiltersCss.text_check}>Показать товары которых нет в наличии</h2>
                        </div>
                        <div className={FiltersCss.scales}>
                            {scales.map(scale => <div className={FiltersCss.scale} key={scale.id + '1'}>
                                    <input type="checkbox" onChange={() => currentScale(scale)} className={FiltersCss.checkbox}
                                           checked={props.currentScale !== null ? props.currentScale.id === scale.id : false} key={scale.id + '2'}/>
                                    <h2 key={scale.id + '3'} className={FiltersCss.scale_text}>{scale.value}</h2>
                            </div>
                            )}
                        </div>
                                ...
                                <div className={FiltersCss.dropdown_list + ' ' + FiltersCss.type_list + ' col-xxl-12 col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12'}>
                                    <div className={FiltersCss.dropdown_items}>
                                        {item.categories.map(category =>
                                            <button key={category.id} className={FiltersCss.bar_list_item + ' col-xxl-12 col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12'}
                                                    onClick={() => {
                                ...
                                <div className={FiltersCss.dropdown_list + ' ' + FiltersCss.brand_list + ' col-xxl-12 col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12'}>
                                    <div className={FiltersCss.dropdown_items}>
                                        {drawCategories.map(categoryDown =>
                                            <button key={categoryDown.id} className={FiltersCss.bar_list_item + ' col-xxl-12 col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12'}
                                ...
                                <div className={FiltersCss.dropdown_list + ' ' + FiltersCss.model_list + ' col-xxl-12 col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12'}>
                                    <div className={FiltersCss.dropdown_items}>
                                        {item.brands.map(brand =>
                                            <button key={brand.id} className={FiltersCss.bar_list_item + ' col-xxl-12 col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12'}
